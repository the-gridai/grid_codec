defmodule GridCodec.BatchTypedFramesDslTest do
  ...
  defmodule MarketCommandsTF do
    use GridCodec.Struct, template_id: 760, schema_id: 70, version: 1
  ...
  defmodule MarketCommandsPU do
    use GridCodec.Struct, template_id: 761, schema_id: 70, version: 1
  ...
  describe "typed_frames DSL integration" do
    test "struct is created with commands field" do
      cmd = %MarketCommandsTF{}
      assert cmd.commands == []
    ...
    test "encode and decode roundtrip" do
      ...
      original = %MarketCommandsTF{
        market_id: <<42::128>>,
        commands: entries
      }

      {:ok, binary} = MarketCommandsTF.encode(original)
      {:ok, decoded} = MarketCommandsTF.decode(binary)

      assert decoded.market_id == <<42::128>>
      assert %GridCodec.Batch{strategy: :typed_frames} = decoded.commands
    ...
    test "O(1) random access via get/2" do
      ...
      original = %MarketCommandsTF{market_id: <<0::128>>, commands: entries}
      {:ok, binary} = MarketCommandsTF.encode(original)
      {:ok, decoded} = MarketCommandsTF.decode(binary)
    ...
    test "empty batch roundtrip" do
      original = %MarketCommandsTF{market_id: <<0::128>>, commands: []}
      {:ok, binary} = MarketCommandsTF.encode(original)
      {:ok, decoded} = MarketCommandsTF.decode(binary)
    ...
    test "header: false roundtrip" do
      ...
      {:ok, with_header} = MarketCommandsTF.encode(original)
      {:ok, without_header} = MarketCommandsTF.encode(original, header: false)

      assert byte_size(without_header) == byte_size(with_header) - 8

      {:ok, decoded} = MarketCommandsTF.decode(without_header, header: false)
      assert GridCodec.Batch.count(decoded.commands) == 2
    end

    test "wire size is smaller than padded_union for varied types" do
      entries = [make_small(1), make_large(2), make_small(3), make_large(4)]

      tf_original = %MarketCommandsTF{market_id: <<0::128>>, commands: entries}
      {:ok, tf_binary} = MarketCommandsTF.encode(tf_original)

      pu_original = %MarketCommandsPU{market_id: <<0::128>>, commands: entries}
      {:ok, pu_binary} = MarketCommandsPU.encode(pu_original)

      assert byte_size(tf_binary) < byte_size(pu_binary)
    end

    test "strategy/1 returns :typed_frames" do
      original = %MarketCommandsTF{market_id: <<0::128>>, commands: [make_small(1)]}
      {:ok, binary} = MarketCommandsTF.encode(original)
      {:ok, decoded} = MarketCommandsTF.decode(binary)

      assert GridCodec.Batch.strategy(decoded.commands) == :typed_frames
    end
  end
end
