defmodule GridCodec.Registry do
  ...
  The consolidated registry generated by the `:grid_codec` Mix compiler rejects
  duplicate `{schema_id, template_id}` pairs and duplicate type names before the
  registry is emitted.

  This fallback registry is less strict:

  - `lookup_by_type/1` warns when duplicate type names exist and keeps the first
    loaded module it sees.
  - wire-ID lookup is built from a runtime map keyed by `{schema_id, template_id}`.
    If duplicate pairs exist in the loaded code set, one module overwrites the
    other in that map instead of raising immediately.

  Because of that, duplicate wire IDs should be treated as invalid application
  state even if the fallback registry happens to resolve one of them.
  """
  ...
  This is the wire-level lookup used by framed GridCodec binaries.
  The lookup key is the pair `{schema_id, template_id}`; `template_id` by itself
  is not globally unique.

  In the consolidated registry this pair is guaranteed unique at compile time.
  In the fallback runtime registry, duplicates are not a supported case and
  ...
  or defaults to the full module path.

  This lookup is separate from wire dispatch. It is useful for integrations
  such as EventStore where events are stored with a stable type string instead
  of a `{schema_id, template_id}` header pair.

  Type names must be unique across loaded GridCodec structs. If you prefer
  short names like `"OrderSubmitted"` instead of the default full module path,
  set `name:` explicitly and ensure it does not collide with another codec.
